                              ...
                              int*        info)
{
    ...
    *info = 0;
    ...
    if (lower)
    {
        ...
        if (*nru > 0)
        {
            F77_FUNC(dlasr, DLASR)("R", "V", "F", nru, n, &work[1], &work[*n], &u[u_offset], ldu);
        }
        if (*ncc > 0)
        {
            F77_FUNC(dlasr, DLASR)("L", "V", "F", n, ncc, &work[1], &work[*n], &c__[c_offset], ldc);
        }
    }

    r__3 = 100.f, r__4 = std::pow(GMX_DOUBLE_EPS, c_b15);
