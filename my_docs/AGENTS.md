...
\n+## �ĵ�ṹ�淶��갻/ǿ�ƣ�
- ʹ�÷�Χ��`my_docs/dev_docs` �� `my_docs/project_docs`
- ��Чʱ�䣺������Ч���������ʽ���ĵ���ƾ�������Σ�

- �½ṹ����
  1) ������`# {����}`��ԭ�ĵ��д��ڱ�������ʹԭ�����⣻�������ڣ��� `{10λʱ���}_{�ļ���}.md` �е������趨��
  2) Ԫ�����ϣ�ʹ�üб�������2 �У�
     - `- ���ߣ�GaoZheng`��Ĭ��ֵ�����Ҫ�����滻
     - `- ���ڣ�YYYY-MM-DD`����Դ�� 10 λ Unix ʱ�����Զ����룬����ָ�����ڣ�
  3) һ���ո�
  4) `## ժҪ` �� 180-240 �ַ���ҪժҪ�����ܣ����ʹ���Զ����ɹ���
  5) ������ԭ�ĵ����ݱ���ԭ���֣����ڱ�ͷ�п��ܡ�### **����**������`#### **ժҪ**` ���������ֱ�����ã������Ԥռ㲻���С�

- O3 ���ע���Զ����룺
  - ����正文���ֳ����¼ؼ��ʣ�`O3����`��`O3Ԫ��ѧ����`��`���纤ά�װ汾��һ�û�ɢ�˶�`��`PFB-GNLA`��
  - ��Լ��λ�ã��Զ����Խű���Ӧ�ڡ����ߡ����ڡ��б���Ԫ����������󣬼����ڡ����ڡ�һ�����󣬼�����`- ���ڣ�YYYY-MM-DD` ��һ�����У�
  - �����ݼ����أ�֧��ԭ�汾��`���ڣ�YYYY��MM��DD��` ��ʽ��������ͬ�������ڵ�һ��
  - ��ע�����幂�Ȳ����أ��������ظ����롣

- ��ͬ�̺�ϵ��
  - �˹淶���滻��ǰ�ļ��еġ����ݹ淶���У��Զ���ڱ��ʽ���涨��
  - ��ʽ�滻�������ڣ�`���ڣ�YYYY��MM��DD��` �����б���`- ���ߣ�...`��`- ���ڣ�YYYY-MM-DD` ��
  - ��Դ�����ĵ���ǰ����ʽ�������ϵ���һ������ɶ�
\n+## 文档命名与时间戳规范（冲突排序）
- 命名格式：`{10位Unix时间戳}_{标题}.md`。10 位前缀既是入库时间戳，也是文档 ID。
- 同秒入库冲突处理：
  - 以“标题名”（去除时间戳前缀与下划线后的文件名部分）按字典序升序排序确定优先级；
  - 排序首项保留原时间戳；其余按排序顺序依次将时间戳向前回退 1 秒（-1s、-2s…），直至唯一；
  - 重命名后需同步更新文档头部为新规范：`- 作者：…`、`- 日期：YYYY-MM-DD`。
- 实施与幂等：由 `my_scripts/align_my_documents.py` 自动执行，重复运行幂等。
- 兼容与豁免：
  - 旧格式 `日期：YYYY年MM月DD日` 将自动迁移为新式“作者/日期”两行；
  - 历史保留的少量前缀按脚本配置（`EXEMPT_PREFIXES`）保留不改名，仅同步头部信息。

## 正文重复分节处理（MUST/SHOULD）
- [必须/MUST] 正文中不得保留与顶部结构重复的分节标题，如“### 标题”“#### 摘要”。
- [必须/MUST] 如原文存在上述分节，必须将其内容合并至：
  - 首行 `# {标题}`（标题并入 H1）；
  - `## 摘要` 段（摘要内容并入该段）。
  合并后删除原“### 标题/#### 摘要”分节，以避免重复与索引歧义。
- [必须/MUST] 顶部固定结构依序为：`# {标题}`、`- 作者：...`、`- 日期：YYYY-MM-DD`、空行、`## 摘要`。
- [应当/SHOULD] 该迁移应由 `my_scripts/align_my_documents.py`/迁移脚本自动完成，保证幂等；人工编辑时亦需遵守。

## 文档命名与时间戳规范（MUST/SHOULD 级别补充）
- [必须/MUST] 命名格式：`{10位Unix时间戳}_{标题}.md`（10 位前缀同时视为文档 ID）。
- [必须/MUST] 同秒入库冲突：按“标题名”字典序升序排序；首项保留原时间戳，其余依次向前回退 1 秒（-1s、-2s…）直至唯一。
