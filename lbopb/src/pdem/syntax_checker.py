...
def default_init_state() -> PDEMState:
...
RULES: Dict[str, Dict[str, str]] = {
    "Identity": {},
    "Bind": {"b": "up", "perim": "up", "f": "up"},
    "Signal": {"b": "up", "perim": "up", "f": "up"},
    "Desensitization": {"b": "down", "perim": "down", "f": "down", "n": "down"},
    "Antagonist": {"b": "down", "perim": "down", "f": "down"},
    "Potentiation": {"b": "up", "perim": "up", "f": "up"},
    "InverseAgonist": {"b": "down", "perim": "down", "f": "down"},
}
...
class RuleEngine:
    def __init__(self) -> None:
        ...
        self.stop_conditions: List[Callable[[PDEMState], bool]] = []
        self.forbidden_pairs: List[Tuple[str, str]] = [
            ("Antagonist", "Potentiation"),
        ]
        self.require_followups: Dict[str, Tuple[int, List[str]]] = {
            "Bind": (3, ["Signal", "Potentiation"]),
            "Signal": (5, ["Desensitization", "Antagonist"]),
        }

    def within_bounds(self, s: PDEMState) -> bool:
...
def check_sequence(seq: List[str], *, init_state: PDEMState | None = None) -> Dict[str, Any]:
    engine = RuleEngine()
    ...
    return {
        "valid": ok,
        "errors": [
            {"index": i, "op": steps[i]["op"] if i < len(steps) else None, "message": msg, "doc": "my_docs/project_docs/1761062406_药效效应幺半群 (PDEM) 公理系统.md"}
...
if __name__ == "__main__":
    ...
    seq = sys.argv[1:] if len(sys.argv) > 1 else ["Bind", "Signal"]
