...
def B(state: TEMState) -> float:
    return float(state.b)


def N_comp(state: TEMState) -> int:
    return int(state.n_comp)


def P(state: TEMState) -> float:
    return float(state.perim)


def F(state: TEMState) -> float:
    return float(state.fidelity)


PhiFunc = Callable[[TEMState], float]
...
class Observables:
    ...
    def eval_all(self, state: TEMState) -> Dict[str, float]:
