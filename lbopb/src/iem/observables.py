...
def B(state: IEMState) -> float:
    return float(state.b)


def N_comp(state: IEMState) -> int:
    return int(state.n_comp)


def P(state: IEMState) -> float:
    return float(state.perim)


def F(state: IEMState) -> float:
    return float(state.fidelity)


PhiFunc = Callable[[IEMState], float]
...
class Observables:
    ...
    def eval_all(self, state: IEMState) -> Dict[str, float]:
