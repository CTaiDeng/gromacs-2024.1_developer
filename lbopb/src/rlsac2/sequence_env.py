...
# 使用 rlsac1 提供的简易空间定义
from lbopb.src.rlsac1.env import SimpleBoxInt32, SimpleBoxFloat32
...
class LBOPBSequenceEnv:
    def __init__(self, case_json: str | Path, *, reward_lambda: float = 0.2) -> None:
        ...
        # obs: module onehot(7) + per-module features(B,P,F,N,risk)*7 + next_op onehot(M) + pos(1)
    ...
    def _single_cost(self, mod: str, st: object, op_name: str) -> float:
        ...
        try:
            if mod == "pem":
                return float(pem_cost([op], st))
            if mod == "pdem":
                return float(pdem_cost([op], st))
            if mod == "pktm":
                return float(pktm_cost([op], st))
            if mod == "pgom":
                return float(pgom_cost([op], st))
            if mod == "tem":
                return float(tem_cost([op], st))
            if mod == "prm":
                return float(prm_cost([op], st))
            if mod == "iem":
                return float(iem_cost([op], st))
        except Exception:
            return 0.0
        return 0.0
