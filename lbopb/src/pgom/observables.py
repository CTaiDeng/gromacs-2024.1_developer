...
def B(state: PGOMState) -> float:
    return float(state.b)


def N_comp(state: PGOMState) -> int:
    return int(state.n_comp)


def P(state: PGOMState) -> float:
    return float(state.perim)


def F(state: PGOMState) -> float:
    return float(state.fidelity)


PhiFunc = Callable[[PGOMState], float]
...
class Observables:
    ...
    def eval_all(self, state: PGOMState) -> Dict[str, float]:
