...
def B(state: PRMState) -> float:
    return float(state.b)


def N_comp(state: PRMState) -> int:
    return int(state.n_comp)


def P(state: PRMState) -> float:
    return float(state.perim)


def F(state: PRMState) -> float:
    return float(state.fidelity)


PhiFunc = Callable[[PRMState], float]
...
class Observables:
    ...
    def eval_all(self, state: PRMState) -> Dict[str, float]:
