...
def B(state: PKTMState) -> float:
    return float(state.b)


def N_comp(state: PKTMState) -> int:
    return int(state.n_comp)


def P(state: PKTMState) -> float:
    return float(state.perim)


def F(state: PKTMState) -> float:
    return float(state.fidelity)


PhiFunc = Callable[[PKTMState], float]
...
class Observables:
    ...
    def eval_all(self, state: PKTMState) -> Dict[str, float]:
