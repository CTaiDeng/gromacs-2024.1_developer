...
def default_init_state() -> PKTMState:
...
RULES: Dict[str, Dict[str, str]] = {
    "Identity": {},
    "Dose": {"b": "up", "perim": "up", "f": "down"},
    "Absorb": {"b": "up", "perim": "up", "f": "down"},
    "Distribute": {"n": "up", "perim": "up", "b": "up"},
    "Metabolize": {"b": "down", "perim": "down", "f": "up"},
    "Excrete": {"b": "down", "perim": "down", "f": "up"},
    # Bind/Transport 方向依上下文而定，不强约束
}


def _instantiate(name: str) -> PKTMOperator:
    mapping = {
        "Identity": Identity,
        "Dose": Dose,
        "Absorb": Absorb,
...
class RuleEngine:
    def __init__(self) -> None:
        ...
        self.stop_conditions: List[Callable[[PKTMState], bool]] = []
        self.forbidden_pairs: List[Tuple[str, str]] = [
            ("Dose", "Dose"),
        ]
        self.require_followups: Dict[str, Tuple[int, List[str]]] = {
            "Dose": (3, ["Distribute", "Absorb"]),
            "Absorb": (5, ["Metabolize", "Excrete"]),
        }

    def within_bounds(self, s: PKTMState) -> bool:
...
def check_sequence(seq: List[str], *, init_state: PKTMState | None = None) -> Dict[str, Any]:
    engine = RuleEngine()
    ...
    return {
        "valid": ok,
        "errors": [
            {"index": i, "op": steps[i]["op"] if i < len(steps) else None, "message": msg, "doc": "my_docs/project_docs/1761062404_药代转运幺半群 (PKTM) 公理系统.md"}
...
if __name__ == "__main__":
    ...
    seq = sys.argv[1:] if len(sys.argv) > 1 else ["Dose", "Distribute", "Metabolize"]
